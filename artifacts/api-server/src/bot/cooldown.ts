...
function scopeKey(scope: CooldownScope) {
  assertDiscordUserId(scope.userId);
  if (scope.guildId) assertDiscordUserId(scope.guildId);
...
export class CooldownService {
  ...
  async check(scope: CooldownScope): Promise<CooldownState> {
    const key = scopeKey(scope);
    const result = await this.pool.query<{ expires_at: Date }>(
      "SELECT expires_at FROM mudae_cooldowns WHERE scope_key = $1",
      [key],
    );
    if (!result.rowCount) return { available: true, remainingMs: 0 };
  ...
  async set(scope: CooldownScope, durationMs: number) {
    assertPositiveInteger(durationMs, "Cooldown duration");
    const key = scopeKey(scope);
    await this.pool.query(
      `INSERT INTO mudae_cooldowns
      ...
      [key, scope.userId, scope.guildId ?? null, scope.action, durationMs],
    );
  }

  async tryAcquire(scope: CooldownScope, durationMs: number): Promise<boolean> {
    assertPositiveInteger(durationMs, "Cooldown duration");
    const key = scopeKey(scope);
  ...
  async clear(scope: CooldownScope) {
    const key = scopeKey(scope);
