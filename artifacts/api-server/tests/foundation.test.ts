...
class EconomyPool {
  ...
  async connect() {
    return {
      query: async (sql: string, params: unknown[] = []): Promise<FakeResult> => {
...
class CooldownPool {
  rows = new Map<string, { expires_at: Date }>();
...
describe("migration planning", () => {
  it("keeps migrations ordered and excludes already-applied IDs", () => {
    assert.deepEqual(migrations.map((migration) => migration.id), ["001", "002", "003", "004", "005", "006", "007", "008", "009"]);
    ...
    assert.match(migrations.find((migration) => migration.id === "008")?.sql ?? "", /3 minutes/);
    assert.match(migrations.find((migration) => migration.id === "009")?.sql ?? "", /canonical_name/);
    assert.deepEqual(
...
describe("economy service", () => {
  it("adds and removes currency atomically while recording transactions", async () => {
    ...
    const economy = new EconomyService(pool as unknown as pg.Pool);
    assert.deepEqual(
      await economy.addCurrency("100000000000000001", 25, { type: "reward" }),
      { ok: true, value: 125 },
    );
    assert.deepEqual(
      await economy.removeCurrency("100000000000000001", 40, { type: "purchase" }),
      { ok: true, value: 85 },
    );
  ...
  it("rejects invalid amounts and insufficient balances", async () => {
    ...
    const economy = new EconomyService(pool as unknown as pg.Pool);
    assert.deepEqual(
      await economy.addCurrency("100000000000000001", 0, { type: "reward" }),
      { ok: false, error: "invalid_amount" },
    );
    assert.deepEqual(
      await economy.removeCurrency("100000000000000002", 11, { type: "purchase" }),
  ...
  it("transfers funds without allowing a negative sender balance", async () => {
    const pool = new EconomyPool();
    const economy = new EconomyService(pool as unknown as pg.Pool);
    const result = await economy.transferCurrency(
      "100000000000000001",
      "100000000000000002",
      30,
      { type: "gift" },
...
describe("cooldown service", () => {
  it("persists cooldown state and reports remaining time until expiration", async () => {
    ...
    const first = new CooldownService(pool as unknown as pg.Pool);
    await first.set(scope, 60_000);
    const second = new CooldownService(pool as unknown as pg.Pool);
  ...
  it("atomically allows only one concurrent acquisition", async () => {
    const pool = new CooldownPool();
    const service = new CooldownService(pool as unknown as pg.Pool);
