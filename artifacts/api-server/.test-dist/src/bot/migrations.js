...
export async function runMigrations(pool) {
  ...
  `);
    ...
    try {
        ...
        await client.query("SELECT pg_advisory_xact_lock(hashtextextended('mudae:schema-migrations', 0))");
        const applied = await client.query("SELECT id FROM mudae_schema_migrations ORDER BY id");
        for (const migration of pendingMigrations(new Set(applied.rows.map((row) => row.id)))) {
            await client.query(migration.sql);
            await client.query("INSERT INTO mudae_schema_migrations (id, name) VALUES ($1, $2)", [migration.id, migration.name]);
        }
