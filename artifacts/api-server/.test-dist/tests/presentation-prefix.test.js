...
import { characterCard, claimedCharacterCard, leaderboardCard, profileCard, remainingCharactersCard, searchResults, } from "../src/bot/presentation.js";
...
describe("character presentation", () => {
    ...
    it("renders character identity and a claim action", () => {
        ...
        assert.match(JSON.stringify(card.components[0].toJSON()), new RegExp(`claim:${roll.id}`));
    });
    it("keeps the curated Mudae artwork, aliases, and stat distribution attached to the catalog", () => {
        ...
        assert.equal(seedCharacters.filter((character) => character.gender === "male").length, 9);
        assert.equal(seedCharacters.filter((character) => character.gender === "female").length, 11);
        assert.ok(Math.min(...seedCharacters.filter((character) => character.rarity === "rare").map((character) => character.value)) >
    ...
    it("uses a configured image URL or an explicit safe fallback", () => {
        ...
        assert.ok(withoutImage.embeds[0].data.fields?.some((field) => field.name === "Artwork" && field.value === "Artwork coming soon"));
    });
    it("renders a claimed character without a second claim action", () => {
        ...
        assert.ok(card.embeds[0].data.fields?.some((field) => field.name === "Claimed By" && field.value === "Player One"));
    });
